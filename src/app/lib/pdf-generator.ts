    ...
    for (const msg of messages) {
      ...
      for (const segment of segments) {
        ...
        for (let i = 0; i < words.length; i++) {
          ...
          const testFont = segment.bold ? boldFont : segment.italic ? italicFont : segment.code ? codeFont : font;
          const testWidth = testFont.widthOfTextAtSize(testText, fontSize);
          const roleWidth = styledLines.length === 0 ? boldFont.widthOfTextAtSize(roleText, fontSize) : 0;
          ...
          if (testWidth <= maxLineWidth - roleWidth) {
            ...
            if (currentLineText) {
              styledLines.push({text: currentLineText, segments: [...currentLineSegments]});
      ...
      for (let lineIndex = 0; lineIndex < styledLines.length; lineIndex++) {
        ...
        let xOffset = lineIndex === 0 ? margin + boldFont.widthOfTextAtSize(roleText, fontSize) : margin;
